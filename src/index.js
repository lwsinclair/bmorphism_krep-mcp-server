...
function findKrepBinary() {
  ...
  if (process.env.DEBUG) {
    ...
    possiblePaths.forEach(p => console.error(`- ${p} (${fs.existsSync(p) ? 'found' : 'not found'})`));
  }
...
app.get('/', (req, res) => {
  res.status(200).json({
    ...
    endpoints: [
      ...
      '/match - Match patterns in strings'
    ],
    algorithms: [
...
app.post('/search', (req, res) => {
  const { pattern, path, caseSensitive = true, threads = 4, countOnly = false } = req.body;
...
// URL route for the MCP URI scheme "search://"
...
// URL route for the MCP URI scheme "match://"
...
if (!fs.existsSync(KREP_PATH) && !process.env.KREP_SKIP_CHECK) {
  ...
  console.error('Please build the krep binary first by running "make" in the krep-native directory');
...
if (require.main === module) {
  ...
  if (process.env.CLAUDE_MCP) {
    ...
    app.listen(PORT, () => {
      console.log(`krep-mcp-server running on port ${PORT}`);
      console.log(`Using krep binary at: ${KREP_PATH}`);
    });
  }
}
